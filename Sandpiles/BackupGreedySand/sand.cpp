...
const char* cSourceFile = "sand2.0.cl";
...
void ppm(int* src, int h, int v, int n, int r, char* s){
    ...
    int res = PPM_RES;
    sprintf(fn,"/home/gpu/vgopalas/sand3/PPMOut/%s%06d.ppm",s,r);
    fp = fopen(fn, "w");
    if(fp ==NULL){
      printf("failure");
      exit(0);
...
void printArray(int* src, int len, int h){
  for(int i = 0; i < len; i++){
    if(i%h ==0){
      printf("\n");
    }
...
int main(int argc, char **argv)
{
  ...
  printArray(sand_i,(graph_h-2)*(graph_v-2),(graph_h-2));
  ...
  int* src = (int*)malloc(graph_n*sizeof(int));
  int* dst = (int*)malloc(graph_n*sizeof(int));
  ...
  //////// INIT DEVICE MEMORY
  cmDevSrc = clCreateBuffer(cxGPUContext, CL_MEM_READ_ONLY, sizeof(cl_int) * graph_n, NULL, &ciErr);
  cmDevDst = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, sizeof(cl_int) * graph_n, NULL, &ciErr2);
  ciErr |= ciErr2;
  ERR("gpu buffers");
  ...
  for(int j = 0; j<runs; j++){
    if(flag){
    ciErr = clEnqueueWriteBuffer(cqCommandQueue, cmDevSrc, CL_TRUE, 0, sizeof(cl_int)*graph_n, src, 1, &event3,&event);
    ERR("write buffer");
    ciErr = clEnqueueWriteBuffer(cqCommandQueue, cmDevDst, CL_TRUE, 0, sizeof(cl_int)*graph_n, dst, 1, &event3,&event4);
    }
    else{
 ciErr = clEnqueueWriteBuffer(cqCommandQueue, cmDevSrc, CL_TRUE, 0, sizeof(cl_int)*graph_n, src, 0, NULL,&event);
 ciErr = clEnqueueWriteBuffer(cqCommandQueue, cmDevDst, CL_TRUE, 0, sizeof(cl_int)*graph_n, dst, 0, NULL,&event4);
    ...
    time += (end-start);
    clEnqueueReadBuffer(cqCommandQueue, cmDevDst, CL_TRUE, 0, sizeof(cl_int)*graph_n, dst, 1, &event2, &event3);
    clFinish(cqCommandQueue);
