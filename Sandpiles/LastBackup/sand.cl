...
void aaup_manager(__global sand* s,int gpos, int ly, int lpos,__local container* rowFlag, __local container* cellFlag, int lsx){
...
void run_once(__global sand* s, int v, int h,__local container* rowFlag,__local container* cellFlag, int ly, int lsx, int lpos,int gpos, int lsy){
         for(int i = 0; i < lsx; i++){
	 	 if(s[gpos+i].val >3){
	 	 ping_update_add(s,gpos+i,lpos+i,ly,lsy,i,lsx,rowFlag,cellFlag,h);
	 }
	 }
	 ...
	 aaup_manager(s,gpos,ly,lpos,rowFlag,cellFlag,lsx);

	 barrier(CLK_GLOBAL_MEM_FENCE);
	 
}
void run_multiple(__global sand* s,int v, int h,__local container* rowFlag,__local container* cellFlag, int ly, int lsx, int lpos, int gpos, int lsy){

     	if(check(rowFlag,cellFlag,ly,lpos,1,0)){
		rowFlag[ly].ping = 0;
		for(int i = 0; i < lsx; i++){
			if(check(rowFlag,cellFlag,ly,lpos+i,0,0)){
			cellFlag[lpos+i].ping = 0;
			ping_update_add(s,gpos+i,lpos+i,ly,lsy,i,lsx,rowFlag,cellFlag,h);
			}
...
__kernel void run(__global sand *s, global container* rCtrl, global container* cCtrl, int v, int h,int lsy,int lsx,int flag){
	 ...
	 int gpos = pos((lsx*nx),get_global_id(1),h);
	 __local container rowFlag[512];
	 __local container cellFlag[512];
	 
	 for(int i = 0; i < lsx; i++){
	 
	 cellFlag[lpos+i].ping = cCtrl[gpos+i].ping;
	 cellFlag[lpos+i].add = cCtrl[gpos+i].add;
	 }
	
	 	 	 
         
	
	 rowFlag[ly].ping = rCtrl[ny*lsy+nx*v+ly].ping;
	 rowFlag[ly].add = rCtrl[ny*lsy+nx*v+ly].add;
	 	 
	 
	 if(!flag){
    	 run_once(s,v,h,rowFlag,cellFlag,ly,lsx,lpos,gpos,lsy);
	 }
	 else{
	 run_multiple(s,v,h,rowFlag,cellFlag,ly,lsx,lpos,gpos,lsy);	
	 }
	 
	
         rCtrl[ny*lsy+nx*v+ly].ping = rowFlag[ly].ping;
	 for(int j = 0; j < lsx; j++){
	 cCtrl[gpos+j].ping = cellFlag[lpos+j].ping;
	 }
