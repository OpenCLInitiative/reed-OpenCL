...
int main(int argc, char **argv)
{
  ...
  int index = 0;
    ...
    ciErr = clEnqueueWriteBuffer(cqCommandQueue, cmDevSrc, CL_TRUE, 0, sizeof(sand)*graph_n, src, 0,NULL,&event[index]);
      ERR("write buffer");
      index ++;
    ...
    clSetKernelArg(kernel, 1, sizeof(int), &runs);
    clSetKernelArg(kernel, 2, sizeof(int),  &graph_h);
    clSetKernelArg(kernel, 3, sizeof(int),  &graph_v);
    clSetKernelArg(kernel, 4, sizeof(int),  &lsy);
    clSetKernelArg(kernel, 5, sizeof(int),  &lsx);
